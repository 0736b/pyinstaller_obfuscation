...
from stringDef.main import Utils
...
class Obfuscate:
    def __init__(self, base_dir=None, compression_level=9):
    ...
    def recompile_bootloader(self):
        ...
        if type(tryCompile) == list:
            logger.error("ERROR COMPILING BOOTLOADER")
            return False
    ...
    def obfuscate(self):
        for directory in os.listdir(self.base_dir):
            if os.path.isdir(directory) and 'pyinstaller-' in directory.lower():
                if directory not in self.base_dir:
                    ...
                    self.pyi_utils = os.path.join(self.bootloader_src_dir, 'pyi_utils.c')
                    self.wscript_file = os.path.join(self.bootloader_dir, 'wscript')
                shutil.copytree(directory, f"{directory}_backup")
        if 'pyinstaller' not in [i.lower() for i in os.listdir(self.base_dir)]:
            sys.exit('[*] COULD NOT FIND pyinstaller dir')
